...
function provision(): string {
  ...
  // One empty vault per test and project, and note what is *not* done to them: no notes are written,
  ...
  for (const project of ["desktop", "mobile"]) {
    for (const purpose of E2E_EMPTY_PURPOSES) {
      const root = emptyVaultRoot(project, purpose);
      mkdirSync(root, { recursive: true });
      run([
        "vault",
        "create",
        "--slug",
        emptyVaultSlug(project, purpose),
        "--name",
        emptyVaultName(project, purpose),
        "--path",
        root,
        "--actor",
        E2E_USER.username,
